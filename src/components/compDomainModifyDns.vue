...
.compDomainModifyDns
  ...
  Alert.alert1(type="warning", show-icon) 温馨提示：
    ...
    template(slot="desc") 1、本功能用于更换执行域名解析和配置的服务器。建议您首先通过新的域名服务器配置好所需的域名解析记录，再进行此项操作；否则将有可能导致您的网站无法被访问。<br />2、如果您选择“使用新网的域名服务器进行解析”，那么您就可以使用新网域名解析功能，在线自助进行A记录、MX记录、CNAME等相关解析记录的操作。<br />建议每天最多修改一次DNS，每次修改需24-48小时才能生效，请耐心等待。<br />4、为保护.gov.cn域名持有者权益，如需修改域名DNS服务器，请通过“提交工单”功能提交具体需求，或直接联系管家。
</template>
